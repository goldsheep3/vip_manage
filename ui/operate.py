from PySide6.QtWidgets import (QWidget, QHBoxLayout, QVBoxLayout, QGroupBox, QMessageBox, QStackedWidget, QTableWidget,
                               QHeaderView, QListWidget)

from lib.n_qt import (LineEdit, Text, DoubleSpinBox, SpinBox, ComboBox, PushButton, GridLayout, VerticalLine)
...
class OperateWidget(QWidget):
    def __init__(self, i18n, conn, app):
        super().__init__()
        self.conn = conn
        self.app = app
        ...
        self.login_stack.addWidget(PushButton('退出', self.on_exit_button_clicked))

        self.card_id.returnPressed.connect(self.on_login_button_clicked)
        self.phone.returnPressed.connect(self.on_login_button_clicked)
        ...
        self.orders = QTableWidget()
        self.orders.setColumnCount(6)
        self.orders.setHorizontalHeaderLabels(['时间', '项目', '金额', '优惠', '实额', '余额'])
        ...
        self.button_modify = PushButton('修改信息', self.on_modify_button_clicked, True)
        self.button_addcny = PushButton('余额储值', self.on_addcny_button_clicked, True)
    ...
    def on_addcny_button_clicked(self):
        pass
